...
在此过程中，Certimate 需要授权凭据信息以控制你的域名解析记录，或访问你的云服务资源。
...
DNS-01 质询是 ACME 协议中用于验证域名所有权的一种方式。通过向域名解析记录中添加一个特定 TXT 记录的方式，ACME 服务器能够查询 DNS 记录来验证用户对域名所有权。如果验证通过，ACME 服务器则签发证书。
