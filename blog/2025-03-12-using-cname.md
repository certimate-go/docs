...
Certimate 底层使用 [go-acme/lego](https://github.com/go-acme/lego) 作为 ACME 库，通过将 `_acme-challenge.你的域名` 设置为指向你控制的验证域名的 CNAME，go-acme/lego 可以在该验证域名上自动创建 TXT 记录，从而完成 ACME 服务器的验证。
